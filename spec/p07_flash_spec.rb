...
describe Flash do
  ...
  describe "#store_flash" do
    ...
    it "adds new cookie with '_maglev_flash' name to response" do
      cookie_str = res.headers['Set-Cookie']
      cookie = Rack::Utils.parse_query(cookie_str)
      expect(cookie["_maglev_flash"]).not_to be_nil
    end

    it "stores the cookie in JSON format" do
      cookie_str = res.headers['Set-Cookie']
      cookie = Rack::Utils.parse_query(cookie_str)
      cookie_val = cookie["_maglev_flash"]
    ...
    it "does not persist data more than 1 request" do
      ...
      second_cookie_val = second_cookie["_maglev_flash"]
  ...
  describe "#[]" do
    it "reads data from flash cookie" do
      cookie = { '_maglev_flash' => { "best_pizza" => "Arizmendi" }.to_json }
